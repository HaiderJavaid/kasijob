...
function ProfileContent() {
  ...
  const [bankDetails, setBankDetails] = useState({ 
    bankName: "Maybank", accountNumber: "", holderName: "" 
  });
  ...
  const profileSteps = [
    { target: '.tutorial-balance', content: 'This is your wallet. We automatically pay you on the 5th of every month!', disableBeacon: true, placement: 'bottom' },
    { target: '.tutorial-bank', content: 'Important: Click here to add your Bank or TNG details so you can receive money.', placement: 'bottom' },
    { target: '.nav-item-leaderboard', content: 'Almost done! Click the Leaderboard to see how to earn a 100% Bonus.', placement: 'top', hideFooter: true, spotlightClicks: true }
  ];
  ...
  const handleTourFinish = async () => {
      ...
      if (user?.uid) {
          try {
              ...
              await updateDoc(userRef, { hasSeenProfileTour: true });
          } catch (e) { console.error("Error saving tour status", e); }
      }
  };
                ...
                <div className="space-y-3">
                    {tasks.length === 0 ? <p className="text-center text-gray-400 text-sm py-4">No tasks completed yet.</p> : tasks.map((task, i) => (
                        <div key={i} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex justify-between items-center">
                            <div><p className="font-bold text-sm text-kasi-dark line-clamp-1">{task.taskTitle}</p><p className="text-xs text-gray-400">Proof: {task.proof?.includes('proofs/') ? 'Image' : 'Text'}</p></div>
                            <div className="text-right"><p className="font-bold text-kasi-gold">+ RM {task.reward?.toFixed(2)}</p><div className="flex justify-end mt-1">{task.status === 'pending' && <span className="text-[10px] bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded font-bold">Reviewing</span>}{task.status === 'approved' && <span className="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded font-bold">Approved</span>}{task.status === 'rejected' && <span className="text-[10px] bg-red-100 text-red-700 px-2 py-0.5 rounded font-bold">Rejected</span>}</div></div>
                        </div>
                    ))}
                </div>
            )}
            ...
            <div className="bg-white w-full max-w-md rounded-3xl p-6 relative z-10 animate-slide-up">
                ...
                <form onSubmit={handleSaveBank} className="space-y-4">
                    <div>
                        ...
                        <select value={bankDetails.bankName} onChange={(e) => setBankDetails({...bankDetails, bankName: e.target.value})} className="w-full bg-gray-50 border-2 border-gray-200 rounded-xl p-3 text-sm font-bold text-kasi-dark outline-none focus:border-kasi-gold">
                    ...
                    <div><label className="block text-xs font-bold text-gray-400 mb-1 uppercase">Account Number / Phone</label><input required type="text" placeholder="e.g. 1122334455" value={bankDetails.accountNumber} onChange={(e) => setBankDetails({...bankDetails, accountNumber: e.target.value})} className="w-full bg-gray-50 border-2 border-gray-200 rounded-xl p-3 text-sm font-bold text-kasi-dark outline-none focus:border-kasi-gold" /></div>
                    <div><label className="block text-xs font-bold text-gray-400 mb-1 uppercase">Account Holder Name</label><input required type="text" placeholder="Must match your KasiJobs name" value={bankDetails.holderName} onChange={(e) => setBankDetails({...bankDetails, holderName: e.target.value})} className="w-full bg-gray-50 border-2 border-gray-200 rounded-xl p-3 text-sm font-bold text-kasi-dark outline-none focus:border-kasi-gold" /></div>
