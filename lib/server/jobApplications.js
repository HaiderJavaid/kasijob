...
export async function markThreadRead(threadId, readerUid) {
  if (!threadId) {
    throw new JobApplicationActionError("Missing thread id.", 400);
  }

  if (!readerUid) {
    throw new JobApplicationActionError("Missing user id.", 401);
  }
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (!Array.isArray(thread.participantIds) || !thread.participantIds.includes(readerUid)) {
      throw new JobApplicationActionError("You are not part of this conversation.", 403);
    }

    transaction.update(
      threadRef,
      new FieldPath("lastReadAtByUser", readerUid),
