...
const normalizeReadMarkers = (value) => {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }

  return Object.entries(value).reduce((markers, [uid, readAt]) => {
    if (uid) {
      markers[uid] = toDate(readAt) || null;
    }
    return markers;
  }, {});
};
...
export function normalizeMessageThread(threadData, id) {
  ...
  const lastMessageAt =
    ...
    null;
  const updatedAt = lastMessageAt || toDate(threadData.updatedAt) || null;
  ...
  return {
    ...
    lastMessageAt,
    lastMessageAuthorId: threadData.lastMessageAuthorId || null,
    lastReadAtByUser: normalizeReadMarkers(threadData.lastReadAtByUser),
...
export async function markMessageThreadRead(threadId) {
  ...
  try {
    const response = await authFetch(`/api/messages/${threadId}/read`, {
      method: "POST",
    });
    ...
    if (!response.ok || !result.success) {
      return { success: false, error: result.error || "Could not mark this conversation read." };
    }

    return result;
  } catch (error) {
    console.error("Error marking message thread read:", error);
    return { success: false, error: error.message };
  }
}
